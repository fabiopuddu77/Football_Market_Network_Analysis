...
for node in g.nodes():
    ...
    if g.nodes()[node]['size'] <50:
        node_trace['marker']['size']+= tuple([g.nodes()[node]['size']/ sum(all_weights)*2])
        node_trace['text'] += tuple(['<b>' + node + '</b>'])
    elif g.nodes()[node]['size'] < 70:
...
fig = go.Figure(
             layout=go.Layout(
                title='Network Graph Football Europe 1990-2021',
                titlefont_size=20,
